...
class SparseConvNeXtV2(nn.Module):
    ...
    def __init__(self, 
        ...
        if self.use_orig_stem:
            self.stem_orig = nn.Sequential(
                MinkowskiConvolution(in_chans, dims[0], kernel_size=patch_size//(2**(self.num_stage-1)), stride=patch_size//(2**(self.num_stage-1)), bias=True, dimension=D), # 8 is a factor of the num_stages -> 2**(num_stages - 1
                MinkowskiLayerNorm(dims[0], eps=1e-6)
            )
        else:
            # an initial conv layer to get dense feature maps
            self.initial_conv = nn.Sequential(
                MinkowskiConvolution(in_chans, dims[0], kernel_size=3, stride=1, bias=True, dimension=D),
                MinkowskiLayerNorm(dims[0], eps=1e-6),
                MinkowskiGELU(),
            )
            # our version of the stem downsampling: includes depthwise_conv + layer norm
            self.stem = nn.Sequential(
                MinkowskiDepthwiseConvolution(dims[0], kernel_size=patch_size//(2**(self.num_stage-1)), stride=patch_size//(2**(self.num_stage-1)), bias=True, dimension=D),
                MinkowskiLayerNorm(dims[0], eps=1e-6)
            )
    ...
    def forward(self, x, mask):
        ...
        scale = int(self.img_size // (num_patches ** .5))
        ...
        for i in range(3):
            ...
            x = self.stages[i+1](x) # the stages var in the code has 4 stages, so the first one is to be used immediately after the stem
