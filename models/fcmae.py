...
class FCMAE(nn.Module):
    ...
    def __init__(
        ...
        self.in_chans = len(args.modalities['sentinel2']) if args.modalities['sentinel2'] != 'all' else len(args.modalities_full['sentinel2'])
        self.out_chans = {}
        ...
        for modality in self.args.out_modalities.keys():
            if modality in ['sentinel2', 'sentinel1', 'aster', 'canopy_height_eth', 'dynamic_world', 'esa_worldcover', 'IMNET']:
                ...
                self.decoder_dict[modality] = nn.Sequential(*decoder)
                self.layer_norm_tmp = LayerNorm(decoder_embed_dim, eps=1e-6, data_format="channels_first")
                self.pred_dict[modality] = nn.Linear(
                    in_features=decoder_embed_dim,
                    out_features=self.out_chans[modality])
    ...
    def forward_loss(self, imgs_dict, preds, mask):
        """
        ...
        for modality in self.args.out_modalities.keys():
            if modality in ['biome', 'eco_region', 'lat', 'lon', 'month', 'era5']:
                ...
                if modality in ['biome', 'eco_region']:
                    ...
                    # we dont need to patchify the image for these modalities
                    # compute the loss 
                    loss = nn.CrossEntropyLoss()(pred, imgs_classes.long())
                    ...
                    imgs = imgs_dict[modality]
                    pred = preds[modality]
                    # we dont need to patchify the image for these modalities but we can still ignore any nan values, all we 
                    nan_mask = torch.isnan(imgs)
                    pred = pred[nan_mask == False]
                    imgs = imgs[nan_mask == False]
                    # compute the loss 
                ...
                if self.norm_pix_loss and modality == 'sentinel2': # we only compute the per-patch norm on sentinel2
                    mean = target.mean(dim=-1, keepdim=True)
                    var = target.var(dim=-1, keepdim=True)
                    target = (target - mean) / (var + 1.e-6)**.5
    ...
    def forward(self, imgs_dict, labels=None, mask_ratio=0.6):
        ...
        # convert nan to 0 for all modalities. This is done since the data is normalized to have a mean of 0 and std of 1. hence 
        # effectively we are setting the nan values to the mean. In the case of the input, setting to 0 also ensures that these
        # values become sparse.  
        for modality in imgs_dict.keys():
            if modality in ['sentinel2', 'sentinel1', 'aster', 'canopy_height_eth']:
                imgs_dict[modality] = torch.nan_to_num(imgs_dict[modality], nan=0.0, posinf=0.0, neginf=0.0)
