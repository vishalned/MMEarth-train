    ...
    def __init__(self, patch_size = 32, img_size = 128, in_chans=3, num_classes=1000, 
        ...
        if self.use_orig_stem:
            ...
            self.stem = nn.Sequential(
                nn.Conv2d(dims[0], dims[0], kernel_size=patch_size//(2**(self.num_stage-1)), stride=patch_size//(2**(self.num_stage-1)), padding = (patch_size//(2**(self.num_stage-1)))//2, groups=dims[0]),
                LayerNorm(dims[0], eps=1e-6, data_format="channels_first")
            )
