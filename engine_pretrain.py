...
def train_one_epoch(model: torch.nn.Module,
    ...
    optimizer.zero_grad()
    for data_iter_step, s in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        normalized_loss_list = normalized_loss_list_.cpu().detach().numpy() if normalized_loss_list_ is not None else None
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, loss_dict, log_var_list, normalized_loss_list