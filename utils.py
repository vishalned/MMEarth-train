...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, best = False):
    ...
    #     save_on_master(to_save, best_path)
        
    
    if is_main_process() and isinstance(epoch, int):
        to_del = epoch - args.save_ckpt_num * args.save_ckpt_freq 
        old_ckpt = output_dir / ('checkpoint-%s.pth' % to_del)
        # uncomment the following line to delete old checkpoints
...
def visualize_segmentation(model, dataloader, device, args, epoch):
    # create directory
    import os
    import random
    import logging
    ...
    # these lines remove the matplotlib logging
    ...
    os.makedirs("/home/qbk152/vishal/global-lr-train/ConvNeXt-V2/visualize", exist_ok=True)

    # get the images, targets and predictions
    images, targets, means, stds = next(iter(dataloader))
    # images, targets, means, stds = next(iter(dataloader))
    ...
    for i in range(N):
        ...
        # for now only visualize the predictions 
        import matplotlib.pyplot as plt

        cmap = plt.cm.get_cmap('viridis', num_classes)
        out = cmap(out / num_classes)[:, :, :3]
        target = cmap(target / num_classes)[:, :, :3]
        ...
        plt.savefig("/home/qbk152/vishal/global-lr-train/ConvNeXt-V2/visualize/epoch_{}_{}.png".format(epoch, i))
        plt.close()